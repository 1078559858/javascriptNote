...
bonusLayer.prototype.checkPlayerWin = function () {
	...
	gGame.gameScene.fBetChoicePrefab.appearWin(gRoomInfo.bonusResult);

	var score = 0;

	var tempArr = gUserInfo.betHistory|| [];

	for(var i = 0; i < tempArr.length; i++){
		if(gRoomInfo.bonusResult[tempArr[i].who]){
			score += Math.round(FormatNumber.twoNumber(tempArr[i].price, 2) * tempArr[i].count);
		}
	}

	this.fTextBonus.text = score;

	this.game.state.states.Net.make1002();
...
bonusLayer.prototype.checkOneWin = function () {
	...
	gGame.gameScene.fBetChoicePrefab.appearWin(1);
};
...
bonusLayer.prototype.checkTwoWin = function () {
	...
	gGame.gameScene.fBetChoicePrefab.appearWin(2);
};
...
bonusLayer.prototype.checkThreeWin = function () {
	...
	gGame.gameScene.fBetChoicePrefab.appearWin(3);
};
...
bonusLayer.prototype.checkFourWin = function () {
	...
	if(max % 2 === 1){
		gGame.gameScene.fBetChoicePrefab.appearWin(4);
	}
};
...
bonusLayer.prototype.checkFiveWin = function () {
	...
	if(max % 2 === 0){
		gGame.gameScene.fBetChoicePrefab.appearWin(5);
	}
};
...
bonusLayer.prototype.checkSixWin = function () {
	...
	if(sub >= 2){
		gGame.gameScene.fBetChoicePrefab.appearWin(6);
	}
};
