...
function ballgroundPrefab(aGame, aParent, aName, aAddToStage, aEnableBody, aPhysicsBodyType) {
	...
	var _yingguo_png = this.game.add.sprite(514, 457, 'bg2', 'yingguo.png', _groupCountry);
	_yingguo_png.scale.setTo(-1.0, 1.0);
	
	var _helan_png = this.game.add.sprite(291, 456, 'bg2', 'helan.png', _groupCountry);
	_helan_png.scale.setTo(-1.0, 1.0);
...
ballgroundPrefab.prototype.updateTime = function () {
	...
	var time = Math.floor(gRoomInfo.duration);

	if(gRoomInfo.state === gRoomConf.state.half4.nick){
		time = (time + 200) >  45000 ? 45000:time;	//加200防止时间不足45S
	}

	if(gRoomInfo.state === gRoomConf.state.half8.nick){
		time = (time + 200) > 90000 ? 90000:time;	//加200防止时间不足90S
	}
